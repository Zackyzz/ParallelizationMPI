...
int main(int argc, char** argv) {
	double time = 0;
	int world_size, world_rank, nrPokemons, nrPikachus = 0, pikachusFound = 0, Pikachu;
	FILE* fPokemons = fopen("C:/Users/desktop/source/repos/findingPik/bam.txt", "r");
	...
	MPI_Bcast(&Pikachu, 1, MPI_INT, 0, MPI_COMM_WORLD);
	...
	if (world_rank != 0) {
		...
		time = MPI_Wtime() - time;
		printf("And there were %d Pikachus, at positions:", nrPikachus);
		for (int i = 0; i < nrPikachus; i++) {
			if (i % 10 == 0)
				printf("\n");
			printf("%d ", *(hapciu + i));
		}
		printf("\nIt took %f s to find all the Pikachus from the list of pokemons.\n", time);
	}
	MPI_Finalize();
}